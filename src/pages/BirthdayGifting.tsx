...
// ✅ Strictly birthday & gift images only
const PRODUCTS = [
  {
    name: 'Celebration Hamper',
    sub: 'Starting at ₹1,499',
    tag: 'Bestseller',
    img: 'https://images.unsplash.com/photo-1549465220-1a8b9238cd48?w=600&q=80', // gift boxes with ribbon
  },
  {
    name: 'Sweet Indulgence Box',
    sub: 'Starting at ₹999',
    tag: 'Customizable',
    img: 'https://images.unsplash.com/photo-1607344645866-009c320b63e0?w=600&q=80', // birthday gift box
  },
  {
    name: 'Luxury Candle Set',
    sub: 'Starting at ₹1,199',
    tag: 'Eco-friendly',
    img: 'https://images.unsplash.com/photo-1464349095431-e9a21285b5f3?w=600&q=80', // birthday cake candles
  },
];
...
const CATEGORIES = [
  {
    ...
    desc: 'Engraved photo frames, custom journals, and memory boxes that turn milestones into treasures.',
    accent: ACCENT,
    tag: 'Personal',
    img: 'https://images.unsplash.com/photo-1513151233558-d860c5398176?w=800&q=80', // birthday balloons gifts
  },
  {
    name: 'Gourmet Celebration Kits',
    desc: 'Artisan chocolates, premium teas, and handcrafted sweets curated for joyful indulgence.',
    accent: '#c4735d',
    tag: 'Gourmet',
    img: 'https://khoyamithai.com/cdn/shop/files/47_14e6fbe7-2b44-400b-ac85-dfea01b2291a.jpg?v=1758631902&width=3003', // gift hamper box
  },
  {
    name: 'Pampering Wellness Sets',
    desc: 'Luxurious bath salts, aromatherapy candles, and skin-care treats for a day of self-love.',
    accent: '#a0617a',
    tag: 'Wellness',
    img: 'https://m.media-amazon.com/images/I/810qdrjodjL._AC_UF350%2C350_QL80_.jpg', // birthday present unwrapping
  },
];
    ...
    <motion.div ref={ref} initial={{ opacity: 0, y: 44 }}
      ...
      <div className="relative overflow-hidden mb-4" style={{ aspectRatio: '1', borderRadius: 16 }}>
        ...
        <span className="absolute top-4 left-4 text-[10px] font-bold uppercase text-white px-4 py-1 rounded-full"
          style={{ background: 'rgba(212,133,106,0.85)', backdropFilter: 'blur(6px)', letterSpacing: 2 }}>{item.tag}</span>
      </div>
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 36 }}
      ...
      <motion.div className="flex items-center justify-center mx-auto mb-5 rounded-full text-3xl"
        ...
        transition={{ duration: 0.3 }}>{item.icon}</motion.div>
    ...
    <div className="overflow-x-hidden" style={{ background: ACCENT_BG, fontFamily: 'inherit' }}>
      ...
      <section ref={heroRef} className="relative flex items-center overflow-hidden" style={{ minHeight: '100vh', perspective: '1400px' }}>
        <motion.div style={{ y: springImg, willChange: 'transform' }} className="absolute inset-0">
          {/* ✅ Birthday balloons & celebration hero */}
          <img
            src="https://images.unsplash.com/photo-1530103862676-de8c9debad1d?w=1600&q=80"
            alt="Birthday Hero" className="w-full h-full object-cover"
            style={{ minHeight: '120%', filter: 'brightness(0.52)' }} />
          ...
          <div className="absolute bottom-0 left-0 right-0 h-28" style={{ background: `linear-gradient(to top,rgba(212,133,106,0.12) 0%,transparent 100%)` }} />
        </motion.div>
        ...
        <motion.div style={{ y: springTxt, opacity: heroOpacity, willChange: 'transform' }}
          ...
          <h1 className="font-serif text-white" style={{ fontSize: 'clamp(3rem,7vw,6rem)', lineHeight: 1.08 }}>
            {['Birthday', 'Gifting'].map((word, i) => (
              <motion.span key={word} initial={{ rotateX: -90, opacity: 0, y: 40 }}
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            Make every birthday feel extraordinary with personalised, sustainable gifts that your team and clients will cherish for years.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 1.05 }} className="flex gap-4 mt-10 flex-wrap">
            <motion.button whileHover={{ scale: 1.05, y: -2, boxShadow: `0 12px 32px rgba(212,133,106,0.45)` }}
              ...
              style={{ background: ACCENT, border: 'none', cursor: 'pointer', fontFamily: 'inherit', boxShadow: `0 8px 30px rgba(212,133,106,0.4)` }}>
              Explore Birthday Gifts
            </motion.button>
            <motion.button whileHover={{ scale: 1.04, y: -2, background: 'white', color: ACCENT_DARK }}
              whileTap={{ scale: 0.96 }} 
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 0.8 }}
          ...
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
            style={{ width: 1, height: 28, background: 'linear-gradient(to bottom,rgba(212,133,106,0.85),transparent)' }} />
        </motion.div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <motion.div variants={vStagger} initial="hidden" animate={aboutInView ? 'show' : 'hidden'}>
            ...
            <motion.p variants={vFadeUp} className="text-lg font-light leading-relaxed mb-5" style={{ color: '#73736e' }}>
              Birthdays are not just dates on a calendar — they are an opportunity to pause, celebrate, and show your team members and clients how much they mean to you. A thoughtful birthday gift speaks louder than any email ever could.
            </motion.p>
            <motion.p variants={vFadeUp} className="text-lg font-light leading-relaxed mb-10" style={{ color: '#73736e' }}>
              We take the guesswork out of corporate birthday gifting with sustainably curated hampers, personalised keepsakes, and gourmet indulgences that make every recipient feel truly seen and valued.
            </motion.p>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <motion.div variants={vSlideLeft} initial="hidden" animate={custInView ? 'show' : 'hidden'}>
            ...
            <p className="text-lg font-light leading-relaxed mb-12" style={{ color: 'rgba(212,212,212,0.7)' }}>
              From the minimalist who appreciates subtle elegance to the one who loves a grand celebration — we craft birthday gifts that mirror their personality perfectly.
            </p>
              ...
              {CUSTOM_ITEMS.map((item) => (
                <motion.div key={item.title} variants={vFadeUp} className="flex gap-4">
                  <div className="flex-shrink-0 flex items-center justify-center text-lg rounded-lg"
                    style={{ width: 40, height: 40, border: '1px solid rgba(240,168,145,0.3)', color: '#f0a891' }}>{item.icon}</div>
                  <div>
      ...
      <section ref={ctaRef} className="py-32 px-8 text-center" style={{ background: '#f5ede9' }}>
        <div className="max-w-4xl mx-auto">
          ...
          <motion.p initial={{ opacity: 0, y: 16 }} animate={ctaInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
            ...
            Let's create personalised birthday experiences that your team will talk about all year long.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={ctaInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
            ...
            <motion.button whileHover={{ scale: 1.06, y: -3, boxShadow: `0 18px 40px rgba(212,133,106,0.35)` }}
            ...
            <motion.button whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 400, damping: 20 }}
              ...
              onClick={() => window.location.href = '/contact'}
              style={{ background: 'transparent', border: 'none', borderBottom: `2px solid rgba(212,133,106,0.3)`, cursor: 'pointer', fontFamily: 'inherit', color: ACCENT_DARK }}
              onMouseEnter={e => (e.currentTarget.style.borderBottomColor = ACCENT)}
              onMouseLeave={e => (e.currentTarget.style.borderBottomColor = 'rgba(212,133,106,0.3)')}>
